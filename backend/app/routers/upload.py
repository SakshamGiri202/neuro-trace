...
async def upload_csv(file: UploadFile = File(...)) -> AnalysisResponse:
    ...
    try:
        ...
        smurfing_accounts = {
            ...
            for result in smurfing_results
        ...
        logger.info(
            f"Analysis completed: {len(suspicious_accounts_list)} suspicious accounts, {len(fraud_rings_list)} fraud rings"
        )
        return response
...
async def get_cytoscape_graph() -> Dict[str, List[Dict[str, Any]]]:
    ...
    if last_analysis_result is None or last_dataframe is None:
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No analysis has been run yet"
        )
    ...
    try:
        ...
        for i in range(G.vcount()):
            ...
            total_sent = sum(G.es[e]["amount"] for e in out_edges) if out_edges else 0
            total_received = sum(G.es[e]["amount"] for e in in_edges) if in_edges else 0

            is_suspicious = node_name in suspicious_account_ids

            node_type = "suspicious" if is_suspicious else "normal"
            ring_id = ring_map.get(node_name, "") if is_suspicious else ""
            score = score_map.get(node_name, 0) if is_suspicious else 0
        ...
        for i, e in enumerate(G.es):
            source_idx = e.source
            target_idx = e.target
            source_name = G.vs["name"][source_idx]
            target_name = G.vs["name"][target_idx]
        ...
        logger.info(
            f"Generated cytoscape graph: {len(nodes)} nodes, {len(edges)} edges"
        )
        ...
        logger.error(
            f"Error generating cytoscape graph: {str(e)}\n{traceback.format_exc()}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
